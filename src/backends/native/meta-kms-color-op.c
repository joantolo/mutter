...
typedef struct _MetaKmsColorOpPropTable
{
  ...
  MetaKmsEnum curve_1d_types[META_KMS_COLOR_OP_1D_CURVE_TYPE_N_PROPS];
  MetaKmsEnum lut_1d_interpolation[META_KMS_COLOR_OP_LUT1D_INTERPOLATION_N_PROPS];
  MetaKmsEnum lut_3d_interpolation[META_KMS_COLOR_OP_LUT3D_INTERPOLATION_N_PROPS];
...
uint32_t
meta_kms_color_op_get_id (MetaKmsColorOp *color_op)
{
  return color_op->id;
}

uint32_t
meta_kms_color_op_get_prop_id (MetaKmsColorOp     *color_op,
                               MetaKmsColorOpProp  prop)
{
  return color_op->prop_table.props[prop].prop_id;
}

const char *
meta_kms_color_op_get_prop_name (MetaKmsColorOp     *color_op,
                                 MetaKmsColorOpProp  prop)
{
  return color_op->prop_table.props[prop].name;
}

uint64_t
meta_kms_color_op_get_prop_drm_value (MetaKmsColorOp     *color_op,
                                      MetaKmsColorOpProp  property,
                                      uint64_t            value)
{
  MetaKmsProp *prop = &color_op->prop_table.props[property];
...
static const char *
color_op_curve_1d_type_to_string (MetaKmsColorOp1DCurveType type)
{
  switch (type)
    {
    case META_KMS_COLOR_OP_1D_CURVE_TYPE_SRGB:
      return "srgb";
    case META_KMS_COLOR_OP_1D_CURVE_TYPE_INV_SRGB:
      return "inv srgb";
    case META_KMS_COLOR_OP_1D_CURVE_TYPE_BT2020:
      return "bt2020";
    case META_KMS_COLOR_OP_1D_CURVE_TYPE_INV_BT2020:
      return "inv bt2020";
    case META_KMS_COLOR_OP_1D_CURVE_TYPE_PQ:
      return "pq";
    case META_KMS_COLOR_OP_1D_CURVE_TYPE_INV_PQ:
      return "inv pq";
    default:
                   ...
                   uint32_t         blob_id)
{
  ...
  /* TODO read properly lut data as gamma array */
...
set_1d_lut_properties (MetaKmsColorOp *color_op)
{
  ...
  prop = &color_op->prop_table.props[META_KMS_COLOR_OP_PROP_SIZE];
  if (prop->prop_id)
    color_op->lut_1d.size = prop->value;

  prop = &color_op->prop_table.props[META_KMS_COLOR_OP_PROP_LUT1D_INTERPOLATION];
  if (prop->prop_id)
    color_op->lut_1d.interpolation = prop->value;
...
set_3d_lut_properties (MetaKmsColorOp *color_op)
{
  ...
  prop = &color_op->prop_table.props[META_KMS_COLOR_OP_PROP_SIZE];
  if (prop->prop_id)
    color_op->lut_3d.size = prop->value;

  prop = &color_op->prop_table.props[META_KMS_COLOR_OP_PROP_LUT3D_INTERPOLATION];
  if (prop->prop_id)
    color_op->lut_3d.interpolation = prop->value;
  ...
  g_message ("JOAN: 3d lut: %u, %p",
             color_op->lut_3d.size,
             color_op->lut_3d.data);
}
...
init_properties (MetaKmsColorOp *color_op)
{
  ...
  *prop_table = (MetaKmsColorOpPropTable) {
      ...
      [META_KMS_COLOR_OP_PROP_TYPE] =
        {
          ...
          .default_value = META_KMS_COLOR_OP_TYPE_1D_CURVE,
        },
      ...
      [META_KMS_COLOR_OP_PROP_1D_CURVE_TYPE] =
        {
          ...
          .num_enum_values = META_KMS_COLOR_OP_1D_CURVE_TYPE_N_PROPS,
          .default_value = META_KMS_COLOR_OP_1D_CURVE_TYPE_SRGB,
        },
      [META_KMS_COLOR_OP_PROP_LUT1D_INTERPOLATION] =
        {
          .name = "LUT1D_INTERPOLATION",
          .type = DRM_MODE_PROP_ENUM,
          .enum_values = prop_table->lut_1d_interpolation,
          .num_enum_values = META_KMS_COLOR_OP_LUT1D_INTERPOLATION_N_PROPS,
          .default_value = META_KMS_COLOR_OP_LUT1D_INTERPOLATION_LINEAR,
        },
      [META_KMS_COLOR_OP_PROP_LUT3D_INTERPOLATION] =
        {
          .name = "LUT3D_INTERPOLATION",
          .type = DRM_MODE_PROP_ENUM,
          .enum_values = prop_table->lut_3d_interpolation,
          .num_enum_values = META_KMS_COLOR_OP_LUT3D_INTERPOLATION_N_PROPS,
          .default_value = META_KMS_COLOR_OP_LUT3D_INTERPOLATION_TETRAHEDRAL,
        },
      [META_KMS_COLOR_OP_PROP_SIZE] =
        {
          .name = "SIZE",
          .type = DRM_MODE_PROP_RANGE,
        },
    ...
    .curve_1d_types = {
      [META_KMS_COLOR_OP_1D_CURVE_TYPE_SRGB] =
        {
          .name = "sRGB EOTF",
        },
    ...
    .lut_1d_interpolation = {
      [META_KMS_COLOR_OP_LUT1D_INTERPOLATION_LINEAR] =
        {
          .name = "Linear",
        },
    },
    .lut_3d_interpolation = {
      [META_KMS_COLOR_OP_LUT3D_INTERPOLATION_TETRAHEDRAL] =
        {
          .name = "Tetrahedral",
        },
    },
