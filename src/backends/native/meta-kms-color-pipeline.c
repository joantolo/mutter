...
uint64_t
meta_kms_color_pipeline_get_id (MetaKmsColorPipeline *color_pipeline)
{
  return color_pipeline->id;
}

GSList *
meta_kms_color_pipeline_get_color_ops (MetaKmsColorPipeline *color_pipeline)
{
  return color_pipeline->color_ops;
}

MetaKmsColorPipeline *
meta_kms_color_pipeline_new (MetaKmsImplDevice  *impl_device,
                             uint64_t            pipeline_id,
                             uint64_t            color_op_id,
                             GError            **error)
{
