  ...
  for (int i = 0; i < G_N_ELEMENTS (test_colors); i++)
    {
      ...
      /* There might be some cases where CPU transforms are outside the
       * [0, 1] range. Clamp them to be in sync with GPU implicit clamping. */
      cpu_color[0] = CLAMP (cpu_color[0], 0.0f, 1.0f);
      cpu_color[1] = CLAMP (cpu_color[1], 0.0f, 1.0f);
      cpu_color[2] = CLAMP (cpu_color[2], 0.0f, 1.0f);
