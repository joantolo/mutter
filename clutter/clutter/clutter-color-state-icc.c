...
{
  clutter_color_op_snippet_append (&sample_3d_lut,
                                   snippet_globals,
                                   snippet_source,
                                   snippet_color_var);
}
...
void
clutter_color_state_icc_do_transform (ClutterColorState *color_state,
                                      ClutterColorState *target_color_state,
                                      const float       *input,
                                      float             *output,
                                      int                n_samples)
{
  ...
  cmsDoTransform (transform, input, output, n_samples);
  cmsDeleteTransform (transform);

  for (i = 0; i < n_samples; i++)
    {
      output[0] = CLAMP (output[0], 0.0f, 1.0f);
      output[1] = CLAMP (output[1], 0.0f, 1.0f);
      output[2] = CLAMP (output[2], 0.0f, 1.0f);
      output += 3;
    }
}
                      ...
                      CoglPixelFormat  *lut_format)
{
  ...
  uint8_t *encoded_output_ptr;
  uint16_t output_half[4];
  size_t bpp;
  int i;
  ...
  if (cogl_context_has_feature (context, COGL_FEATURE_ID_TEXTURE_HALF_FLOAT))
    {
      ...
      encoded_output_ptr = g_malloc (n_samples * bpp);
      *encoded_output = encoded_output_ptr;
      *lut_format = COGL_PIXEL_FORMAT_RGBX_FP_16161616;

      for (i = 0; i < n_samples; i++)
        {
          output_half[0] = cogl_float_to_half (lut_output[0]);
          output_half[1] = cogl_float_to_half (lut_output[1]);
          output_half[2] = cogl_float_to_half (lut_output[2]);
          output_half[3] = cogl_float_to_half (1.0f);
          memcpy (encoded_output_ptr, output_half, bpp);
          encoded_output_ptr += bpp;
          lut_output += 3;
        }
  ...
  *lut_format = COGL_PIXEL_FORMAT_RGBX_8888;
  ...
  for (i = 0; i < n_samples; i++)
    {
      encoded_output_ptr[0] = (uint8_t) (lut_output[0] * UINT8_MAX);
      encoded_output_ptr[1] = (uint8_t) (lut_output[1] * UINT8_MAX);
      encoded_output_ptr[2] = (uint8_t) (lut_output[2] * UINT8_MAX);
      encoded_output_ptr[3] = (uint8_t) UINT8_MAX;
      encoded_output_ptr += bpp;
      lut_output += 3;
    }
}
            ...
            Clutter3DLut         **out_lut_3d)
{
  ...
  g_autofree float *lut_output = NULL;
  ...
  lut_output = g_malloc (n_samples * 3 * sizeof (float));

  sample_3d_lut_input (&lut_input, lut_size);

  clutter_color_state_icc_do_transform (CLUTTER_COLOR_STATE (color_state_icc),
                                        CLUTTER_COLOR_STATE (target_color_state_icc),
                                        lut_input,
                                        lut_output,
                                        n_samples);

  encode_3d_lut_output (lut_output,
                        n_samples,
                        &encoded_lut_output,
                        &lut_format);
                             ...
                             int           layer_index)
{
  ...
  switch (lut_3d->format)
    ...
    case COGL_PIXEL_FORMAT_RGBX_8888:
      ...
      break;
    default:
      g_warning ("Unhandled pixel format uploading 3D lut");
      return FALSE;
    }
  ...
  cogl_pipeline_set_layer_texture (pipeline, layer_index, lut_texture);
  ...
  cogl_pipeline_set_layer_combine (pipeline, layer_index,
                                         ...
                                         CoglPipeline      *pipeline)
{
  ...
  if (!upload_3d_lut_as_2d_texture (pipeline,
                                    lut_3d,
...
clutter_color_state_icc_class_init (ClutterColorStateIccClass *klass)
{
  ...
  color_state_class->do_transform = clutter_color_state_icc_do_transform;
...
get_transform_matrices (ClutterColorStateParams *color_state_params,
                        cmsFloat64Number         to_pcs_rel[9],
                        cmsFloat64Number         to_rgb_rel[9])
{
  ...
  to_pcs_rel[0] = graphene_matrix_get_value (&to_pcs_merged, 0, 0);
  to_pcs_rel[1] = graphene_matrix_get_value (&to_pcs_merged, 1, 0);
  to_pcs_rel[2] = graphene_matrix_get_value (&to_pcs_merged, 2, 0);
  to_pcs_rel[3] = graphene_matrix_get_value (&to_pcs_merged, 0, 1);
  to_pcs_rel[4] = graphene_matrix_get_value (&to_pcs_merged, 1, 1);
  to_pcs_rel[5] = graphene_matrix_get_value (&to_pcs_merged, 2, 1);
  to_pcs_rel[6] = graphene_matrix_get_value (&to_pcs_merged, 0, 2);
  to_pcs_rel[7] = graphene_matrix_get_value (&to_pcs_merged, 1, 2);
  to_pcs_rel[8] = graphene_matrix_get_value (&to_pcs_merged, 2, 2);
  ...
  to_rgb_rel[0] = graphene_matrix_get_value (&to_rgb_merged, 0, 0);
  to_rgb_rel[1] = graphene_matrix_get_value (&to_rgb_merged, 1, 0);
  to_rgb_rel[2] = graphene_matrix_get_value (&to_rgb_merged, 2, 0);
  to_rgb_rel[3] = graphene_matrix_get_value (&to_rgb_merged, 0, 1);
  to_rgb_rel[4] = graphene_matrix_get_value (&to_rgb_merged, 1, 1);
  to_rgb_rel[5] = graphene_matrix_get_value (&to_rgb_merged, 2, 1);
  to_rgb_rel[6] = graphene_matrix_get_value (&to_rgb_merged, 0, 2);
  to_rgb_rel[7] = graphene_matrix_get_value (&to_rgb_merged, 1, 2);
  to_rgb_rel[8] = graphene_matrix_get_value (&to_rgb_merged, 2, 2);
}
                 ...
                 cmsToneCurve             *inv_eotf_curves[3])
{
  ...
  for (i = 0, t = 0.0f; i < n_points; i++, t += step)
    {
      values[i] = clutter_eotf_apply (*eotf, t);
      inv_values[i] = clutter_eotf_apply_inv (*eotf, t);
    }
  ...
  if (!eotf_curve || !inv_eotf_curve)
    {
      ...
      cmsFreeToneCurve (eotf_curve);
      cmsFreeToneCurve (inv_eotf_curve);
...
clutter_color_state_icc_new_from_params (ClutterColorState *color_state)
{
  ...
  cmsStage *stage;
  cmsPipeline *D_to_B_1, *B_to_D_1;
  cmsFloat64Number to_pcs_rel[9], to_rgb_rel[9];
  ...
  /* Relative rendering intent (DtoB1/BtoD1) */
  /* TODO: Add Perceptual intent and others */
  D_to_B_1 = cmsPipelineAlloc (NULL, 3, 3);
  stage = cmsStageAllocToneCurves (NULL, 3, eotf_curves);
  cmsPipelineInsertStage (D_to_B_1, cmsAT_END, stage);
  stage = cmsStageAllocMatrix (NULL, 3, 3, to_pcs_rel, NULL);
  cmsPipelineInsertStage (D_to_B_1, cmsAT_END, stage);
  cmsWriteTag (icc_profile, cmsSigDToB1Tag, D_to_B_1);

  B_to_D_1 = cmsPipelineAlloc (NULL, 3, 3);
  stage = cmsStageAllocMatrix (NULL, 3, 3, to_rgb_rel, NULL);
  cmsPipelineInsertStage (B_to_D_1, cmsAT_END, stage);
  stage = cmsStageAllocToneCurves (NULL, 3, inv_eotf_curves);
  cmsPipelineInsertStage (B_to_D_1, cmsAT_END, stage);
  cmsWriteTag (icc_profile, cmsSigBToD1Tag, B_to_D_1);

  cmsPipelineFree (D_to_B_1);
  cmsPipelineFree (B_to_D_1);
